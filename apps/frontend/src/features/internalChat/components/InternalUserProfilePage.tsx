...
import { ArrowLeft, Camera, BookOpen, MessageSquare, Trophy, Calendar, Mail, Shield, ChevronRight, Volume2 } from 'lucide-react';
...
interface InternalUserProfileData {
  name: string;
  bio: string;
  avatar: string;
  joinedDate: string;
...
interface InternalUserProfilePageProps {
  user: InternalUserProfileData;
  onBack: () => void;
}

export function InternalUserProfilePage({ user, onBack }: InternalUserProfilePageProps) {
  return (
      ...
      <div className="max-w-4xl mx-auto w-full p-6 space-y-8 pb-12">
        <section className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center md:items-start gap-8">
          <div className="relative group">
            ...
            <button className="absolute bottom-0 right-0 p-2.5 bg-blue-600 text-white rounded-full border-4 border-white shadow-lg hover:bg-blue-700 transition-colors" aria-label="Alterar foto">
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div>
              ...
              <p className="text-blue-600 font-semibold text-sm mt-1">Nivel: Curador da Alma</p>
            </div>
            <p className="text-gray-500 text-sm leading-relaxed max-w-lg">{user.bio}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 flex flex-col items-center text-center">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Debates</p>
          </div>
          <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 flex flex-col items-center text-center">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Obras Citadas</p>
          </div>
          <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100 flex flex-col items-center text-center">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Mencoes</p>
          </div>
        </div>

        <div className="bg-white rounded-[32px] overflow-hidden shadow-sm border border-gray-100">
          <div className="p-6 border-b border-gray-50">
            <h3 className="font-bold text-gray-800">Configuracoes Literarias</h3>
          </div>
          <div className="divide-y divide-gray-50">
            {[
              { label: 'Preferencias de Voz', desc: 'Ajuste o timbre dos autores', icon: Volume2 },
              { label: 'Notificacoes de Grupo', desc: 'Gerencie alertas de novos debates', icon: MessageSquare },
              { label: 'Privacidade e Visibilidade', desc: 'Quem pode ver seu perfil', icon: Shield },
              { label: 'E-mail e Comunicacao', desc: 'Newsletter de recomendacoes', icon: Mail },
            ].map((item, i) => {
